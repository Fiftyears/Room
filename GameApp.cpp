...
void GameApp::UpdateScene(float dt)
{
	...
	if (m_KeyboardTracker.IsKeyPressed(Keyboard::D1) && m_CurrMode != ShowMode::WoodCrate)
	{
		...
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pbrick.GetAddressOf());
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pfloor.GetAddressOf());
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pgrass.GetAddressOf());*/
	}
	...
	else if (m_KeyboardTracker.IsKeyPressed(Keyboard::D3) && m_CurrMode != ShowMode::TextureBox)
	{
		// 播放木箱动画
		m_CurrMode = ShowMode::TextureBox;
		m_pd3dImmediateContext->IASetInputLayout(m_pVertexLayout3D.Get());
		auto meshData = Geometry::CreateBox();
		ResetMesh(meshData);
		m_pd3dImmediateContext->VSSetShader(m_pVertexShader3D.Get(), nullptr, 0);
	...
	else if (m_CurrMode == ShowMode::TextureBox)
	{
		...
		m_VSConstantBuffer.worldInvTranspose = XMMatrixInverse(nullptr, W);	// 两次转置抵消
	     ...
	     m_VSConstantBuffer.world = XMMatrixTranspose(texMat);*/
...
void GameApp::DrawScene()
{
	...
	if (m_pd2dRenderTarget != nullptr)
	{
		...
		static const WCHAR* textStr = L"切换显示: 1-木箱(3D) 2-animation(2D) \n"
		L"3-业火的向日葵";

		static const WCHAR* textStr2 = L"2020QG工作室图形组第4次小组培训\n"
		 L"\t\t\t刘熠\n"
		"\t\t      2020.4.23";

		m_pd2dRenderTarget->DrawTextW(textStr, (UINT)wcslen(textStr), m_pTextFormat.Get(),
			D2D1_RECT_F{ 0.0f, 0.0f, 600.0f, 0.0f }, m_pColorBrush.Get());
		m_pd2dRenderTarget->DrawTextW(textStr2, (UINT)wcslen(textStr2), m_pTextFormat.Get(),
			D2D1_RECT_F{ 800.0f, 0.0f, 480.0f, 200.0f }, m_pColorBrush.Get());
...
bool GameApp::InitEffect()
{
	...
	/*HR(CreateShaderFromFile(L"HLSL\\FlareBox_VS.cso", L"HLSL\\FlareBox_VS.hlsl", "VS", "vs_5_0", blob.ReleaseAndGetAddressOf()));
...
bool GameApp::InitResource()
{
	...
	// 初始化木箱纹理
	m_pWoodCrate.resize(7);
	m_pFlare.resize(2);
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\flare.dds", nullptr, m_pWoodCrate[0].GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\brick.dds", nullptr, m_pWoodCrate[1].GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\flarealpha.dds", nullptr, m_pWoodCrate[2].GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\floor.dds", nullptr, m_pWoodCrate[3].GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\grass.dds", nullptr, m_pWoodCrate[4].GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\WoodCrate.dds", nullptr, m_pWoodCrate[5].GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\flareoutputA.dds", nullptr, m_pWoodCrate[6].GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\flare.dds", nullptr, m_pFlare[0].GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\flarealpha.dds", nullptr, m_pFlare[1].GetAddressOf()));
	
	// 初始化火焰纹理
	WCHAR strFile[40];
	m_pFireAnims.resize(24);
	for (int i = 1; i <= 24; ++i)
	{
	...
	m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pWoodCrate[0].GetAddressOf());
	m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pWoodCrate[2].GetAddressOf());
	//m_pd3dImmediateContext->PSSetShaderResources(0, 2, m_pFlare[0].GetAddressOf());
	m_pd3dImmediateContext->PSSetShader(m_pPixelShader3D.Get(), nullptr, 0);
	...
	/*D3D11SetDebugObjectName(m_pPixelShader3D.Get(), "Flare_PS");
	D3D11SetDebugObjectName(m_pVertexShader3D.Get(), "Flare_VS");*/
